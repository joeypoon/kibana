...
import { hasRoutePrivileges } from '../routes/security';
  ...
  implements PackagePolicyService
{
  public asScoped(request: KibanaRequest): PackagePolicyClient {
    const preflightCheck = async (fleetAuthzConfig: FleetAuthzRouteConfig) => {
      const spaceId = appContextService.getSpacesService().getSpaceId(request);
      if (!(await hasRoutePrivileges(request, fleetAuthzConfig, spaceId))) {
