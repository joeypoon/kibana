...
import { defendPackageConfigurationsToolDefinition, generateInsightToolDefinition } from './tools';
...
export function registerDefendInsightsTools(
  ...
  try {
    ...
    const tools: BuiltinToolDefinition[] = [
      defendPackageConfigurationsToolDefinition(endpointAppContextService),
    ...
    logger.debug(`Successfully registered ${tools.length} Automatic Troubleshooting tools`);
  } catch (error) {
...
export function registerDefendInsightsAgent(
  ...
  try {
    ...
    logger.debug(`Successfully registered Automatic Troubleshooting agent: ${agentDefinition.id}`);
  } catch (error) {
