...
You are a leading expert on resolving Elastic Defend configuration issues. Your task is to review the data below and provide an accurate and detailed step by step solution to the Elastic Defend configuration issue described in the problem description. Organize your response precisely to the following rules:
...
`,
  ...
  CUSTOM: {
    ...
    REFINE: `
You previously generated the below insights using this prompt: ${DEFEND_INSIGHTS_CUSTOM_PROMPT}.
Double check the generated insights below and make sure it adheres to the rules set in the original prompt, removing events only as necessary to adhere to the original rules. In addition:
- combine duplicate insights into the same 'group'
- remove insights with no events
    `,
