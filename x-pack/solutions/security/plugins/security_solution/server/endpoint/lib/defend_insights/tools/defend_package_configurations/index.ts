...
import { DEFEND_PACKAGE_CONFIGURATIONS_TOOL_ID } from '../../constants';
import {
...
export const defendPackageConfigurationsToolDefinition = (
  endpointAppContextService: EndpointAppContextService
): BuiltinToolDefinition<typeof defendPackageConfigurations> => {
  return {
    id: DEFEND_PACKAGE_CONFIGURATIONS_TOOL_ID,
    type: ToolType.builtin,
    description: `Fetches Elastic Defend package configurations.

Before using this tool, ensure you have gathered relevant context from the ${platformCoreTools.integrationKnowledge} tool.
...
- When there is potential misconfiguration in the Elastic Defend package assets`,
    ...
    handler: async (_, { esClient, logger }) => {
      ...
      if (installation) {
        packageData = await packageClient.getPackage(FLEET_ENDPOINT_PACKAGE, installation.version);
      }
      let defaultTransformSettings: Array<{ path: string; config: unknown }> = [];
      if (packageData) {
      ...
      // are there are concerns using internal user here?
