...
import { DefendInsightType, DefendInsights } from '@kbn/elastic-assistant-common';
...
const generateInsightSchema = z.object({
  ...
  // timeStart: z.string().describe('Start time of the analysis period in ISO 8601 format.'),
  // timeEnd: z.string().describe('End time of the analysis period in ISO 8601 format.'),
...
- To create actionable insights for resolving Elastic Defend configuration issues`,
    ...
    handler: async (
      ...
      { modelProvider, logger, toolProvider, request }
    ) => {
      // const generateEsql = await toolProvider.get({
      //   toolId: platformCoreTools.generateEsql,
      //   request,
      // });
      // const foo = await generateEsql.execute({ toolParams: {nlQuery: 'bar' } });
