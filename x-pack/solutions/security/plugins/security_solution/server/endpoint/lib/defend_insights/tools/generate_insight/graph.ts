...
import type { ScopedModel, ToolHandlerResult } from '@kbn/onechat-server';
...
Provide a concise and actionable insight for each group of events that can help address the problem described.
  ...
  async function createWorkflowInsights({ insights, insightType }: StateType) {
    ...
    const workflowInsights = await securityWorkflowInsightsService.createFromAgentDefendInsights(
      insights,
      endpointIds,
      insightType,
      model
